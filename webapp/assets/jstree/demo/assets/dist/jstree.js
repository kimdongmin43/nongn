 ...
 */
	...
	$.jstree.reference = function (needle) {
		...
		if(!obj || !obj.length) {
			try { obj = $(needle); } catch (ignore) { console.log(ignore, 'error 발생');}
		}
		if(!obj || !obj.length) {
			try { obj = $('#' + needle.replace($.jstree.idregex,'\\$&')); } catch (ignore) { console.log(ignore, 'error 발생');}
		}
	...
	$.jstree.core.prototype = {
		...
		destroy : function (keep_html) {
			if(this._wrk) {
				...
				catch (ignore) { console.log(ignore, 'error 발생');}
			}
						...
						if(document.selection && document.selection.empty) {
							...
							if(window.getSelection) {
								...
								try {
									sel.removeAllRanges();
									sel.collapse();
								} catch (ignore) { console.log(e, 'error 발생');}
							}
						}
					})
				...
				try {
					...
					if(!this._data.core.working || force_processing) {
						...
						w.onmessage = $.proxy(function (e) {
							...
							try { w.terminate(); w = null; } catch(ignore) { console.log(ignore, 'error 발생');}
	...
	$.jstree.plugins.search = function (options, parent) {
		...
		this._search_open = function (d) {
			...
			$.each(d.concat([]), function (i, v) {
				if(v === "#") { return true; }
				try { v = $('#' + v.replace($.jstree.idregex,'\\$&'), t.element); } catch(ignore) { console.log(ignore, 'error 발생');}
				if(v && v.length) {
					if(t.is_closed(v)) {
