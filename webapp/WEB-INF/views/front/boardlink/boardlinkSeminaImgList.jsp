			...
			<ul>
				...
				<li>
					...
					<c:if test="${not empty list.fileNm}">
						<a href="javascript:seminaView('${list.contId }')"><img src="http://www.korcham.net/FileWebKorcham/OnlineSeminar/Image/${list.fileNm}" alt="썸네일" width="100%" height="100%" /></a>
					</c:if>
					<c:if test="${empty list.fileNm}">
						<img src="/images/front/common/detail_no_img.png" alt="이미지 없음" class="info_img" />
					</c:if>
					</div>
					<a href="javascript:seminaView('${list.contId }')" class="gallaylist_txt">
						<h4>${list.title }</h4>
						<div class="date">[${fn:replace(list.regDt,'/','.') }]</div>
