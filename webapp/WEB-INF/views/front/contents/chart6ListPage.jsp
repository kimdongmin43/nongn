...
<script>
	var IndicatorsUrl =  "<c:url value='/front/contents/chart6ListPage.do'/>";
	var gridUrl = "<c:url value='/front/contents/chartDownload.do'/>";
	//var gridUrl = "<c:url value='/front/contents/chartDownload.do'/>";
	var gubunArr = [];

	<c:forEach var="codelist" items="${codelist}" varStatus="loop">
	gubunArr[${loop.count}] = '${codelist.m1}/${codelist.gubun}';
	</c:forEach>
	...
	function makeM2(str){
		...
		$("#searchB").append("<option value=''>소분류</option>");
...
<div class="content">
	...
	<div class="tab_style">
		<ul>
			<li class="w2"><a href="/front/contents/chart5ListPage.do?menuId=5368">농작물</a></li>
			<li class="w2"><a href="#none" class="active" title="선택됨">가축</a></li>
		</ul>
	</div>
	<!-- //tab_style -->
	<div class="cont_search_area">
		<div class="cont_search_box w100">
			<form name="listFrm"  id="listFrm" method="post" onsubmit="return false;">
				<input type="hidden" name="searchType"  id="searchType" value="612">
				<input type="hidden" name="searchYM"  id="searchYM">
				<input type="hidden" name="menuId"  value="${param.menuId}">
				<div class="con_wrap">
					<dl>
						<dt><label for="searchA">축종</label></dt>
						<dd class="step_con">
							<fieldset>
								<select id="searchA" name = "searchA" class = "select_style" data-parsley-required="true" onchange="makeM2($(this).val());">
								...
								<select id="searchB" name = "searchB" class = "select_style" data-parsley-required="true" title="축종 소분류">
	...
	<div class="date_choice fl_right">(단위: 천두, 백만원)</div>
	<div class="table_auto_area table_wrap scroll">	
		<table class="table_style_auto">
			<caption>가축 보험료수준 정보표로 지역,가입두수,가입농가수,영업보험료,국고지원,시도지원,시군구지원,농가부담,천두당영업보험료(원),천두당농가부담보험료(원) 정보를 제공함.</caption>
			<colgroup>
				<col width="12%" />
				<col width="8%" />
				<col span="2" width="10%" />
			...
			<thead>
				<tr>
					...
					<th scope="col">천두당<br/>영업보험료(원)</th>
					<th scope="col">천두당<br/>농가부담보험료(원)</th>
				</tr>
			</thead>
			<tbody>
				...
				<tr>
					...
					<td><fmt:formatNumber value="${chartlist2.v3}" pattern="#,###" /></td>
					<td><fmt:formatNumber value="${chartlist2.v4}" pattern="#,###" /></td>
					<td><fmt:formatNumber value="${chartlist2.v5}" pattern="#,###" /></td>
	...
	<p class="mt10">* 보험사업자 (농협손보, KB손보, DB손보, 한화손보, 현대해상) 제공 데이터를 기반으로 산출되었습니다.<br />
	<span style="color:#ea4200;">* 위 보험료는 실적 기준의 평균 보험료이며, 가입금액 및 개인별 할인·할증률에 따른 보험료가 달라질 수 있습니다.<br /></span>
	* 축종별 지역별 세부 자료는 엑셀 다운로드를 하시면 보실 수 있습니다.</p>
</div>
...
<script>
	...
	function excelDown(){
		if(jq){dn()};
		setTimeout("exportExcel('Chart6')",1000);
	}
	 function dn(){
		 ...
		 $('#event_list').jqGrid({
				...
				colModel: [
					...
					{ label: '천두당 영업보험료(원)', index: 'v8', name: 'v8',  align : 'center', sortable:false},
					{ label: '천두당 농가부담보험료(원)', index: 'v9', name: 'v9', align : 'center', sortable:false }
				],
				postData :{
					searchTxt : $("#searchTxt").val(),
					searchYM : $('#searchKey').val(),
					//chartKey : '6',
					// 2018.04.03 : 엑셀출력시 중분류, 소분류 조건 적용
	...
	//excel로 출력
	 function  exportExcel( pFileName ) {
		  ...
		  var html = "";
		  html+="<table>";
		  html+="<tr><td colspan='12' bgcolor='#ffff99'>[주석]<br><br>1. 보험사업자(농협손보, KB손보, 한화손보, DB손보, 현대해상) 제공 데이터를 기반으로 산출되었습니다.<br>";
		  ...
		  html+="<font color='red'>*실적 기준의 평균 보험료이며, 가입금액 및 개인별 할인ㆍ할증률에 따라 보험료가 달라질 수 있습니다.</font></tr>";
		  html+="<tr><td colspan='12' align='right'>(단위:천두, 백만원)</td></tr>";
		  html+="</table>";
