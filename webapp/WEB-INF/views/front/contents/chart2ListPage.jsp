...
<script>
	var IndicatorsUrl =  "<c:url value='/front/contents/chart2ListPage.do'/>";
...
<div class="content">
	...
	<div class="tab_style">
		<ul>
			<li class="w2"><a href="/front/contents/chart1ListPage.do?menuId=5366">품목별</a></li>
			<li class="w2"><a href="#none" class="active" title="선택됨">지역별</a></li>
		</ul>
	</div>
	<!-- //tab_style -->
	...
	<div class="cont_search_area">
		<div class="cont_search_box">
			<form name="listFrm"  id="listFrm" method="post" onsubmit="return false;" onchange="changeDecember();">
				<input type="hidden" name="searchType"  id="searchType" value="605">
	...
	<div class="table_auto_area table_wrap scroll">
		<table class="table_style_auto">
			<caption>지역별 농작물재해보험 정보표로 지역,가입면적,가입농가수,가입금액,순보험료,지급보험금,가입률 정보를 제공함.</caption>
			<colgroup>
				<col width="22%" />
				<col span="6" width="13%" />
			</colgroup>
			...
			<tbody>
				...
				<tr>
					...
					<td><fmt:formatNumber value="${chartlist2.v8}" pattern="0.0" />%</td>
				</tr>
				</c:forEach>
...
<script>
	...
	function excelDown(){
		if(jq){dn()};
	//	setTimeout("exportExcel('Chart2')",5000);
	}
	 function dn(){
		 ...
		 $('#event_list').jqGrid({
				...
				mtype: 'POST',
				colModel: [
					{ label: '시도', index: 'c1', name: 'c1',  align : 'center', sortable:false},
					...
					{ label: '가입농지수', index: 'v4', name: 'v4',  align : 'left', sortable:false},
	...
	//excel로 출력
	 function  exportExcel( pFileName ) {
		  ...
		  var html = "";
		  html+="<table>";
		  html+="<tr><td colspan  ='9' bgcolor='#ffff99'>[주석]<br><br>1. 보험사업자(농협손보) 제공 데이터를 기반으로 산출되었습니다.<br>";
		  ...
		  html+="<tr><td colspan  ='9' align='right'>(단위:ha, 호, 건, 백만원, ％)</td></tr>";
		  html+="</table>";
...
<script>
	$(document).ready(function(){		//	웹 품질 진단 조치 - 2022.02.22
		...
		$("#txtTitle").text(strTitleText + " > 지역별");
	});
</script>
