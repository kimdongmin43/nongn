...
%>
		...
		<div class="search_item">
        	<div class="search_tit">
                <h4>게시물 [ <span id="Lcnt">${totalcnt}</span>건 ]</h4>
                <c:if test="${totalcnt>3}">
                <a href="javascript: searchAll('L')" class="btn_search_add" id="more2" title="게시물 더보기">
                    <span>더보기 +</span>
                </a>
                </c:if>
            </div>
            <ul>
            	...
            	<li>&#183; "<span style="color:#f47251; font-weight:700;">${param.searchTxt}</span>"에 대한 검색결과가 없습니다.<br><%  // 검색결과 없음 %>
            	...
            	<li>
                    <a href="/front/board/boardContentsView.do?contId=${searchinfo.contId}&menuId=${searchinfo.menuId}" target="_blank" title="해당 컨텐츠로 새창 이동" style="display:block;">
                        ...
                        <p class="search_word">
	                    	...
	                    	<% String Txt = request.getAttribute("text").toString().replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>", "").replaceAll("&nbsp;"," ");%>
							<%=subString(Txt,0,500)%>
                        </p>
                    </a>
                </li>
		...
		<!--// list_table_area -->
		<div id="Lpage" style="display:none;">
		<!-- paging_area -->
		${boardPagging}
		<!--// paging_area -->
		</div>