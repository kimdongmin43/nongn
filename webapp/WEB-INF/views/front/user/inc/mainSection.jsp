						...
						<ul class="tab_type2">
							...
							<li<c:if test="${status.index eq 0}"> class="active"</c:if><c:if test="${empty row.boardId}"> class="none"</c:if>>
								...
								<c:when test="${row.boardId eq 3}">
									<!--// 3. 온라인세미나 -->
									<a class="more" href="/front/boardlink/boardlinkContentsListPage.do?boardId=3&menuId=${row.menuId}"><img src="/images/more.png" alt="더보기" /></a>
								</c:when>
								<c:when test="${row.boardId eq 4}">
									<!--// 4. 행사/교육 -->
									<a class="more" href="/front/event/eventList.do?menuId=${row.menuId}"><img src="/images/more.png" alt="더보기" /></a>
								</c:when>
								<c:when test="${row.boardId < 20}">
									<a class="more" href="/front/boardlink/boardlinkContentsListPage.do?boardId=${row.boardId}&menuId=${row.menuId}"><img src="/images/more.png" alt="더보기"></a>
								</c:when>
								<c:otherwise>
									<!--  일반게시판 -->
						...
						<div class="tab-content">
							...
							<c:choose>
								<c:when test="${row.boardId eq 1}">
									<!--  1.경제지표 -->
										...
										<ul class="nav tab_type1">
											...
											<li <c:if test="${tempDiv eq ''}" >class="active"</c:if>>
												<a href="${eRow.moreUrl}" data-href="#tab${row.sectionCd}_${eRow.divCd}" target="_blank">${eRow.divNm}</a>
											</li>
								...
								<c:when test="${row.boardId eq 3}">
									...
									<a href="/front/boardlink/boardlinkSeminaView.do?menuId=3886&boardId=3&contId=${seminarMap.seqNoD010}" >
								...
								<c:when test="${row.boardCd eq 'A' or row.boardCd eq 'W'}">
									...
									<li class="mainPhotoNews">
										...
										<div class="mainPhotoNewsTitle"><a href="${sRow.refUrl}<c:if test="${not empty sRow.refUrl}">&menuId=${row.menuId}</c:if>">${sRow.title}</a><span class="mPn_day">${sRow.regDt}</span></div>
								...
								<c:when test="${row.boardId eq 18}">
									<!--// 발간자료 -->
									<ul class="boardlist_t3">
										<c:set var="cnt" value="0"/>
										<c:forEach  var="sRow" items="${mainBoardContentsList}" varStatus="status1">
											<c:if test="${row.sectionCd eq sRow.sectionCd and row.tabSeq eq sRow.tabSeq and cnt<2}">
												<li>
													<%-- <div class="date">${sRow.regDt}</div> --%>
													<a href="${sRow.refUrl}<c:if test="${not empty sRow.refUrl}">&menuId=${row.menuId}</c:if>" class="board_tit"><img class="img" src="http://www.korcham.net/FileWebKorcham/PublishData/Image/${sRow.urlLink}" alt="${sRow.title}"  width="60" hright="100" style="border:1px solid #dedede; margin: 5px 10px 15px 40px;"/></a>
													<div style="float:right; margin-top:15px;width:190px; height:50px; oveflow:hidden;">
													<a href="${sRow.refUrl}<c:if test="${not empty sRow.refUrl}">&menuId=${row.menuId}</c:if>" class="board_tit">
													${fn:replace(sRow.title,'-','<br/>-')}
													</a>
													</div>
												</li>
												<c:set var="cnt" value="${cnt+1}"/>
