...
<script>
var korchamBoardSearchList = "<c:url value='/back/board/korchamBoardSearchList.do'/>";
...
function selBoard(){
	...
	$("#refKorBoardId").val(ret.boardId);
	$("#refBoardNm").val(ret.title);
	$("#refKorBoardNm").val(ret.title);
	$("#refCd_K").find("input[id=refBoardNm]").val(ret.title);
	
	$("#menuNm").val(ret.title);
	$("#originMenuNm").val(ret.title);
	$("#refUrl").val(ret.refUrl);
	$("#refCd_K").find("input[name=url]").val($("#refUrl").val());
	popupSearchClose();
}
